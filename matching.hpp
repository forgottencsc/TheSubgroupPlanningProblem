...
private:
    ...
    private:
        ...
        void up_heap(int pos) {
            for (int k = pos; k > 1 && node[k] < node[k / 2]; k >>= 1)
                swap_node(k, k / 2);
        }
        int size_;
        vector<Node> node;
...
private:
        ...
        for (int b = N + 1, r = B - unused_bid_idx_; r > 0 && b < S; ++b)
            if (base[b] != b) {
                if (surface[b] == b) {
                    fix_mate_and_base(b);
                    if (label[b] == kOuter)
                        potential[b] += (time_current_ - time_created[b]) * 2;
    ...
    void contract(int x, int y, int eid) {
        ...
        for (int d = 0; d < 2; ++d) {
            for (int bv = surface[x]; bv != lca;) {
                ...
                if (bv > N)
                    potential[bv] += (time_current_ - time_created[bv]) * 2;
                if (bmv > N)
                    heap4.erase(bmv);
        ...
        if (lca > N)
            potential[lca] += (time_current_ - time_created[lca]) * 2;
    ...
    bool grow(int root, int x, int y) {
        ...
        heap2.erase(by);
        if (y != by)
            heap4.update(by, time_current_ + (potential[by] / 2));
        int z = mate[by];
        ...
        for (int b = old_base;; b = node[b].link[d].b) {
            ...
            if (b > N) {
                if (potential[b] == 0)
                    ...
                    heap4.push(b, time_current_ + (potential[b] / 2));
            }
            if (b == root)
                break;
...
vector<vertex_t> minimum_weighted_matching(const graph_t &g) {
    ...
    for (auto [it, end] = edges(g); it != end; ++it) {
        ...
        es.push_back(MWM::InputEdge{(int)src + 1, (int)dst + 1, n * max_w - w + 1});
    }
