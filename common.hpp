...
private:
  ...
  private:
    ...
    void up_heap(int pos) {
      for (int k = pos; k > 1 && node[k] < node[k / 2]; k >>= 1) swap_node(k, k / 2);
    }
    int size_;
    vector<Node> node;
    vector<int> index;
...
private:
  ...
  void reset_all() {
    ...
    for (int b = N + 1, r = B - unused_bid_idx_; r > 0 && b < S; ++b) if (base[b] != b) {
      if (surface[b] == b) {
        fix_mate_and_base(b);
        if (label[b] == kOuter) potential[b] += (time_current_ - time_created[b]) * 2;
  ...
  void contract(int x, int y, int eid) {
    ...
    for (int d = 0; d < 2; ++d) {
      for (int bv = surface[x]; bv != lca; ) {
        ...
        if (bv > N) potential[bv] += (time_current_ - time_created[bv]) * 2;
    ...
    if (lca > N) potential[lca] += (time_current_ - time_created[lca]) * 2;
  ...
  bool grow(int root, int x, int y) {
    ...
    if (y != by) heap4.update(by, time_current_ + (potential[by] / 2));
    int z = mate[by];
  ...
  void expand(int bid) {
    ...
    for (int b = old_base; ; b = node[b].link[d].b) {
      ...
      if (b > N) {
        ...
        else heap4.push(b, time_current_ + (potential[b] / 2));
      }
      if (b == root) break;
  ...
  bool augment(int root) {
    ...
    while (!que.empty()) {
      ...
      for (int eid = ofs[x]; eid < ofs[x + 1]; ++eid) {
        ...
        Label l = label[by];
        if (l == kOuter) {
          cost_t t = reduced_cost(x, y, e) / 2; // < 2 * Inf
          if (t == time_current_) {
...
vector<vertex_t> minimum_weighted_matching(const graph_t& g) {
    ...
    auto p = MWM(n, es).maximum_weighted_matching(false);
    vector<vertex_t> mate(n, graph_t::null_vertex());
    for (size_t i = 0; i < n; ++i)
        mate[i] = p.second[i + 1] - 1;
    return mate;
}
